...
export function ProjectListItem({ project, onOpen }: Props) {
  ...
  return (
    <div
      ...
      className="bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-lg p-3 cursor-pointer hover:bg-zinc-850 transition-colors flex items-center justify-between gap-4 text-xs group"
    >
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <button
          onClick={handleToggleFavorite}
          className="text-zinc-500 hover:text-amber-400 p-1 rounded shrink-0 transition-colors"
        ...
        <div
          className={cn(
            ...
            isRunning ? 'bg-emerald-500/20 text-emerald-400' : 'bg-zinc-800 text-zinc-400'
          )}
        >
        ...
        <div className="hidden sm:flex items-center gap-2 w-32 shrink-0">
          {project.project_type && (
            <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-zinc-800 text-zinc-300 font-mono uppercase">
        ...
        <div className="hidden lg:flex items-center gap-1.5 flex-1 min-w-0">
          {project.languages.map((l) => (
            <span
              key={l}
              className="px-1.5 py-0.2 bg-emerald-950/40 border border-emerald-800/30 text-emerald-300 text-[10px] rounded shrink-0"
            >
              {l}
            </span>
          ))}
          {project.frameworks.map((f) => (
            <span
              key={f}
              className="px-1.5 py-0.2 bg-blue-950/40 border border-blue-800/30 text-blue-300 text-[10px] rounded shrink-0"
            >
              {f}
            </span>
          ))}
            ...
            <button
              ...
              className={cn(
                ...
                isRunning
                  ...
                  : 'bg-zinc-800 text-emerald-400 hover:bg-emerald-950/80'
              )}
